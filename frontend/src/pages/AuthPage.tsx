...
const AuthPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      login(data.user); // assumes backend returns { user, token }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 flex items-center justify-center py-12 px-4 relative">
      {/* Back arrow */}
      <Link to="/" className="absolute left-8 top-8 flex items-center text-gray-500 hover:text-enteru-600 transition-colors">
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 relative">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <Button type="submit" className="w-full text-base font-semibold" loading={loading}>Login</Button>
        </form>
        ...
        <div className="mt-6 text-center text-gray-600">
          Don&apos;t have an account? <Link to="/signup" className="text-enteru-600 font-semibold hover:underline">Sign Up</Link>
        </div>
      </div>
    </div>
  );
